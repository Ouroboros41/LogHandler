...
log_directory = r"C:/Program Files/MiningTag/MT Production LoopCounter/production/logs"
service_name = "MT Production LoopCounter"
...
def restart_service(service_name):
    """Reinicia el servicio, con un límite de intentos por hora."""
    global restart_attempts
    current_time = time.time()
    
    # Filtrar reinicios de la última hora
    restart_attempts = [t for t in restart_attempts if current_time - t < 1800]
      
    # Evitar reinicios si el último fue hace menos de MIN_TIME_BETWEEN_RESTARTS segundos
    if current_time - last_restart_time < MIN_TIME_BETWEEN_RESTARTS:
        logging.warning(f"Reinicio evitado: se detectó otro error en menos de {MIN_TIME_BETWEEN_RESTARTS} segundos.")
        return

    if len(restart_attempts) >= MAX_RESTARTS_PER_HOUR:
        logging.warning(f"Límite de reinicios alcanzado ({MAX_RESTARTS_PER_HOUR}/h). No se reiniciará el servicio.")
        return

    try:
        subprocess.run(["powershell", "-Command", f"Stop-Service -Name '{service_name}'"], check=True)
        logging.info(f"Servicio '{service_name}' detenido con éxito.")
        time.sleep(1)
        ...
        restart_attempts.append(current_time)  # Registrar intento
        last_restart_time = current_time  # Actualizar última marca de tiempo de reinicio
...
def monitor_log_file():
    ...
    event_handler = LogHandler(current_log_file, block_time=10)  # segundos de bloqueo ajustables
    observer = Observer()
