...
class LogHandler(FileSystemEventHandler):
    def __init__(self, log_file):
        ...
        self.error_block_time = 0  # Tiempo de bloqueo para errores
    ...
    def check_for_errors(self):
        """Revisa el log en busca del último error y reinicia el servicio si se detecta alguno."""
        try:
            with open(self.log_file, "r") as file:
                ...
                for line in new_lines:
                    ...
                    # Ignorar errores si se esta en tiempo de bloqueo
                    if current_time < self.error_block_time:
                        continue

                    # Buscar "ERROR" en la línea
                    if "|ERROR|" in line:
                        ...
                        if timestamp_match:
                            ...
                            try:
                                ...
                                # Verificar si es un nuevo error
                                if timestamp > self.last_error_timestamp and timestamp > start_time:
                                    ...
                                    # Establecer tiempo de bloqueo de 1 segundo
                                    self.error_block_time = current_time + 1
                                    break
                            except ValueError:
...
def monitor_log_file():
    ...
    event_handler = LogHandler(current_log_file)
    observer = Observer()
        ...
        while True:
            ...
            if expected_log_file != current_log_file:
                ...
                if not expected_log_file:
                    logging.error("No se encontró un archivo de log para el nuevo día. Continuando monitorización del archivo actual.")
                    continue

                # Actualizar el archivo de log monitorizado
                observer.unschedule_all()
                event_handler = LogHandler(expected_log_file)
                observer.schedule(event_handler, log_directory, recursive=False)
